...
algorithmsToRemove = ['DBSCAN', 'Birch','HDBSCAN','AffinityPropagation','MeanShift','GaussianMixture',]
...
for name in cluster_solution_names:
    ...
    mini_df[""] = mini_df[""]+1
    mini_df.to_csv("All Results\\Folder 1\\Clustering Results\\"+name+".csv",index=False,header=False)
...
for i in range(2,11):
    ...
    #mini_df[""] = mini_df[""]
    mini_df.to_csv("All Results\\Folder 1\\Clustering Results\\mixture_model_{}.csv".format(str(i)),index=False,header=False)
...
for threshold in closure_thresholds:
    ...
    mini_df[""] = mini_df[""] + 1
    mini_df.to_csv("All Results\\Folder 1\\Clustering Results\\Graph_closure threshold_{}.csv".format(str(threshold)),index=False,header=False)
...
result_df.to_csv("All Results\\Folder 1\\Total Results\\total_results.csv")
