...
THIS FILE IS IMPORTED AND USED AS PART OF BATCHINTERNAL.PY
"""
import numpy as np
...
class internalIndex:
    ...
    def SD_valid (self, data, label):
        ...
        max_cent_dist =  np.max(sy.spatial.distance.pdist(centroid_list,'euclidean'))
        min_cent_dist = np.min(sy.spatial.distance.pdist(centroid_list,'euclidean'))
        seperation = 0
